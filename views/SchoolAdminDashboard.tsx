...
import { 
  ...
  UserPlus, CreditCard, UserCheck, Briefcase, Clock, Printer, BellRing, ReceiptText, ChevronDown
} from 'lucide-react';
                  ...
                  <tr key={stu.id} className="hover:bg-slate-50/50 transition-all">
                    ...
                    <td className="px-8 py-6 text-center">
                      <span className={`px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                        stu.feeStatus === 'Paid' ? 'bg-emerald-50 text-emerald-600' : 
        ...
        {activeTab === 'dash' && (
          <div className="space-y-8 animate-in fade-in duration-500">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
               {[
                 { l: 'Admissions', v: '12 New', i: UserPlus, c: 'indigo' },
                 { l: 'Hostel Occupancy', v: '84%', i: Building, c: 'emerald' },
                 { l: 'Canteen Credit', v: '$2.4k', i: CreditCard, c: 'rose' },
                 { l: 'Staff Attendance', v: '100%', i: UserCheck, c: 'amber' },
               ].map((s, i) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
               <div className="lg:col-span-2 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8">
                  <div className="flex justify-between items-center mb-8">
                     <h3 className="text-xl font-black text-slate-900 uppercase">Recent Activity Hub</h3>
                     <button className="px-4 py-2 bg-slate-50 text-slate-400 text-[10px] font-black uppercase rounded-xl">View All logs</button>
                  </div>
                  <div className="space-y-4">
                     {[
                       { m: 'Bus #04 delayed by 10 mins', t: '5 mins ago', cat: 'Transport' },
                       { m: 'New admission approved for Grade 10B', t: '1 hour ago', cat: 'Admissions' },
                       { m: 'Laboratory stock low - Chemicals', t: '3 hours ago', cat: 'Inventory' }
                     ].map((a, i) => (
                       <div key={i} className="p-4 bg-slate-50 rounded-2xl flex justify-between items-center">
                          <div className="flex items-center gap-4">
                             <div className="w-2 h-2 rounded-full bg-indigo-500"></div>
                             <span className="text-sm font-bold text-slate-700">{a.m}</span>
                          </div>
                          <span className="text-[10px] font-black uppercase text-slate-400">{a.t}</span>
                       </div>
                     ))}
                  </div>
               </div>
               <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden flex flex-col justify-center">
                  <Megaphone size={120} className="absolute top-0 right-0 opacity-10 p-4" />
                  <h4 className="text-2xl font-black mb-4 uppercase tracking-tighter">Branch Bulletin</h4>
                  <p className="text-slate-400 text-sm mb-8 italic">Release terminal announcements strictly to this node.</p>
                  <button onClick={() => triggerToast('Opening announcement terminal...')} className="w-full py-4 bg-indigo-600 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl shadow-indigo-900/40">Broadcasting Terminal</button>
               </div>
            </div>
          </div>
        )}
